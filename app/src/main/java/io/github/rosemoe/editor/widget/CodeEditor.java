...
public class CodeEditor extends View implements ContentListener, TextAnalyzer.Callback, FormatThread.FormatResultReceiver, LineRemoveListener {
	...
	private boolean typefacedCompletion;
	private boolean typefacedLines;
...
*/
    ...
    /**
	 * Set the custom count for tabs.
	 * It will be used in auto addition of tabs by editor
     */
	public void setTabCount(int count) {
	  mCursor.setTabCount(count);
	}
    /**
    ...
    private void drawView(Canvas canvas) {
        ...
        Log.d(LOG_TAG, "Drawing view took:" + timeUsage + "ms");
    ...
    private void drawText(Canvas canvas, char[] src, int index, int count, float offX, float offY) {
        ...
        if (st < end) canvas.drawText(src, st, end - st, offX, offY, mPaint);
    }
    ...
    public void updateCompletionWindowPosition() {
        ...
        if (restY > mDpUnit * 200) {
			...
			if (isAllowFullScreen()) {
				offset += getRowHeight() / 2;
			}
            getScroller().startScroll(getOffsetX(), getOffsetY(), 0, (int) offset, 1000);
        }
        mCompletionWindow.setExtendedX(panelX);
        mCompletionWindow.setExtendedY(panelY);
        if (getWidth() < 500 * mDpUnit) {
            //Open center mode
            mCompletionWindow.setWidth(WindowManager.LayoutParams.WRAP_CONTENT/*getWidth() * 7 / 8*/);
        ...
        mCompletionWindow.setMaxHeight(400);
    ...
    public void beginSearchMode() {
        class SearchActionMode implements ActionMode.Callback {
            ...
            public boolean onCreateActionMode(ActionMode p1, Menu p2) {
                ...
                p2.add(0, 0, 0, "Next").setShowAsActionFlags(MenuItem.SHOW_AS_ACTION_IF_ROOM);
                p2.add(0, 1, 0, "Last").setShowAsActionFlags(MenuItem.SHOW_AS_ACTION_NEVER);
                p2.add(0, 2, 0, "Replace").setShowAsActionFlags(MenuItem.SHOW_AS_ACTION_NEVER);
                p2.add(0, 3, 0, "Replace All").setShowAsActionFlags(MenuItem.SHOW_AS_ACTION_NEVER);
     ...
     */
    public void setTypeface(Typeface typefaceText) {
    ...
    /**
	 * @return Whether to set typeface on AutoCompleteWindow or not
     * @see CodeEditor#setTypefacedCompletion(boolean)
	 */
	public boolean isTypefacedCompletion() {
		return this.typefacedCompletion;
	}

	/**
	* Warning: You need to set editor's typeface first
	* Set if to apply typeface on completion window
    *
	* @param value whether to apply or not
    */
	public void setTypefacedCompletion(boolean value) {
		this.typefacedCompletion = value;
	}

    /**
	 * @return Whether to set typeface on lineCountLayout or not
     * @see CodeEditor#setTypefacedLines(boolean)
	 */
	public boolean isTypefacedLines() {
		return this.typefacedLines;
	}

	/**
	* Warning: You need to set editor's typeface first
	* Set if to apply typeface on LineCountLayout
    *
	* @param value whether to apply or not
    */
	public void setTypefacedLines(boolean value) {
		this.typefacedLines = value;
	}
    /**
